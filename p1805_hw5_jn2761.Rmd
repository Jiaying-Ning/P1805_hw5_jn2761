...
The sample average of mu across tests for which the null is rejected is not equal to true value of mu for mu equal to 0 and 1, but it is approxitemently equal for mu equal to 2 to 6. This is because since we have a null hypothesis of mu equal to 0, we will be more likely reject sample that have means far away from 0. Thus for sample that have true mean equal to 0 and 1, we will be more likely to reject samples that have deviated sample mean, whereas for sample that have much higher true mu value, most of the sample mean will be consider as deviated means for null hypohthesis mu = 0, thus even for the rejected sample, their mean will be similar to the true mean.
