...
tibble(
  ...
  mutate(
    output_list = map(.x = mu_c-1,~rerun(1000,sim_norm(mu=.x))), #function inside of a function #rerun a 100 times
...
sample_results%>%
  ...
  filter(decision=="reject") %>%
  summarise(n_reject=n()) %>%
  ggplot(aes(x=mu_c, y=n_reject,fill=mu_c)) + 
  geom_bar(stat = "identity") +
  scale_x_continuous(
    breaks = c(0:6))+ 
  labs(titles = "the number of time null-hypothesis is rejected",
        x = "true value of μ",
       y= "number of rejection with null mean=0") +
